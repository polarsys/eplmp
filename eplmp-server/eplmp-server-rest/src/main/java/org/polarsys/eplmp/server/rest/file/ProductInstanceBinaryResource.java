...
public class ProductInstanceBinaryResource {
    ...
    public Response uploadFilesToProductInstanceIteration(
        ...
        try {
            ...
            if (formParts.size() == 1) {
                return org.polarsys.eplmp.server.rest.file.util.BinaryResourceUpload.tryToRespondCreated(request.getRequestURI() + URLEncoder.encode(fileName, "UTF-8"));
            }
            return Response.noContent().build();

        } catch (IOException | ServletException | StorageException e) {
            return org.polarsys.eplmp.server.rest.file.util.BinaryResourceUpload.uploadError(e);
        }
